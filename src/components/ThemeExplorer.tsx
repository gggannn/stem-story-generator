...
import { ThemeCategory, Topic, ThemeDomain, ParticleShape } from '@/types';
...
// Merge themeDomains with shader data
const CATEGORIES: CategoryData[] = themeDomains.map((domain) => {
  const shaderMap: Record<string, any> = {
    science: ScienceShaderMaterial,
    technology: TechShaderMaterial,
    engineering: EngineeringShaderMaterial,
    earth: EarthShaderMaterial,
    math: MathShaderMaterial,
  };

  const particleConfig: Record<string, { color: string; shape: ParticleShape; ring: boolean }> = {
    science: { color: '#a78bfa', shape: 'atom', ring: false },
    technology: { color: '#22d3ee', shape: 'pixel', ring: true },
    engineering: { color: '#fb923c', shape: 'block', ring: false },
    earth: { color: '#34d399', shape: 'drop', ring: false },
    math: { color: '#f472b6', shape: 'crystal', ring: true },
  };

  const config = particleConfig[domain.id] || { color: '#ffffff', shape: 'atom' as ParticleShape, ring: false };

  return {
    id: domain.id as ThemeCategory,
    name: domain.name,
    icon: domain.icon,
    color: domain.id,
    hexColor: domain.color,
    shaderMaterial: shaderMap[domain.id] || ScienceShaderMaterial,
    topics: domain.subThemes.map((st) => ({ id: st.id as Topic, name: st.name })),
...
function EpicTopicParticle({
  ...
  const getGeometry = () => {
    const scale = 0.8; // 整体缩小以适应移动端
    switch (category.particleShape) {
      case 'atom': return new THREE.SphereGeometry(0.28 * scale, 32, 32);
      case 'pixel': return new THREE.BoxGeometry(0.24 * scale, 0.24 * scale, 0.24 * scale);
      case 'block': return new THREE.BoxGeometry(0.3 * scale, 0.18 * scale, 0.18 * scale);
      case 'drop': return new THREE.ConeGeometry(0.16 * scale, 0.32 * scale, 16);
      case 'crystal': return new THREE.OctahedronGeometry(0.2 * scale, 0);
      default: return new THREE.SphereGeometry(0.28 * scale, 32, 32);
    }
  };
...
function EnhancedTopicParticles({
  ...
  const topicPositions = useMemo(() => {
    ...
    // 根据主题数量动态调整半径，数量越少半径越小
    const baseRadius = count <= 3 ? 1.3 : count <= 5 ? 1.5 : 1.8;

    for (let i = 0; i < count; i++) {
      // 改为 2D 圆形分布，Z 轴更接近 0，减少遮挡
      ...
      positions.push({
        ...
        position: [
          ...
          Math.sin(angle * 2) * 0.1, // 轻微波动增加层次感，但不遮挡
        ],
      });
    }
    return positions;
...
export function ThemeExplorer({ selectedTopic, onTopicChange }: ThemeExplorerProps) {
  ...
  return (
    <div
      className="relative w-full h-[550px] rounded-2xl overflow-hidden border border-slate-800/60 shadow-2xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:ring-offset-2 focus:ring-offset-slate-900"
      tabIndex={0}
      onKeyDown={(e) => {
        if (e.key === 'Escape' && phase !== 'categories') {
          handleBack();
        }
      }}
    >
      ...
      {phase !== 'categories' && (
        <button
          onClick={handleBack}
          className="absolute top-5 left-5 flex items-center gap-2 px-5 py-2.5 bg-black/60 backdrop-blur-md rounded-full text-white/90 text-sm font-medium hover:bg-black/75 transition-all border border-white/15 hover:border-white/25 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:ring-offset-2 focus:ring-offset-slate-900"
        >
