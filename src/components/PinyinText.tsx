...
export function PinyinText({ text, className = '' }: PinyinTextProps) {
  ...
  while (i < text.length) {
    ...
    if (isChinese(char)) {
      ...
      // Punctuation: wrap in a span with baseline alignment to stay with Chinese characters
      elements.push(
        <span key={key++} className="pinyin-punct">{char}</span>
      );
    } else {
      // All other characters: spaces, symbols - render as plain text
      elements.push(char);
    }
    i++;
  }
